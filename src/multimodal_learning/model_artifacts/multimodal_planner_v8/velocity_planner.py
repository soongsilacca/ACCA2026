...
def build_mpc_path_from_candidate(
    route_xy_m: np.ndarray,
    base_candidate_xy_m: np.ndarray | None = None,
    ...
    *,
    ...
    """Apply the model candidate beginning at its actual first route point."""
    route, cumulative, origin_s = _route_from_origin(route_xy_m)
    if route_start_index is not None:
        start_index = int(route_start_index)
        if start_index < 0 or start_index >= len(route):
        ...
        origin_s = float(cumulative[start_index])
    if interval_m <= 0.0 or lateral_deadband_m < 0.0:
        raise ValueError("path interval must be positive and deadband nonnegative")

    path_start_progress = 0.0
    candidate_station = np.empty(0, dtype=np.float64)
    candidate_keep_indices = np.empty(0, dtype=np.int64)
    candidate_offset = np.empty((0, 2), dtype=np.float64)

    if base_candidate_xy_m is not None or candidate_xy_m is not None:
        base_candidate = np.asarray(base_candidate_xy_m, dtype=np.float64)
        candidate = np.asarray(candidate_xy_m, dtype=np.float64)
        if (
            base_candidate.ndim != 2
            or base_candidate.shape[1] != 2
            or candidate.shape != base_candidate.shape
            or len(candidate) < 2
            or not np.isfinite(base_candidate).all()
            or not np.isfinite(candidate).all()
        ):
            raise ValueError("candidate paths must be matching finite [N,2]")

        # Project each model base point directly onto the Local Route.  The
        # first projection is the actual beginning of the MPC path; no
        # synthetic (s=0, delta_d=0) point is inserted ahead of it.
        segments = np.diff(route, axis=0)
        lengths = np.maximum(np.linalg.norm(segments, axis=1), 1.0e-6)
        fraction = np.clip(
            np.sum(
                (base_candidate[:, None, :] - route[None, :-1, :])
                * segments[None, :, :],
                axis=2,
            )
        ...
        projected = (
            route[None, :-1, :]
            + fraction[:, :, None] * segments[None, :, :]
        )
        nearest_segment = np.argmin(
            np.sum(
                np.square(base_candidate[:, None, :] - projected),
                axis=2,
            ),
            axis=1,
        )
        rows = np.arange(len(base_candidate))
        candidate_station = (
            cumulative[nearest_segment]
            + fraction[rows, nearest_segment] * lengths[nearest_segment]
            - origin_s
        )
        candidate_station = np.maximum.accumulate(candidate_station)
        candidate_offset = candidate - base_candidate
        candidate_offset[
            np.linalg.norm(candidate_offset, axis=1) <= lateral_deadband_m
        ] = 0.0

        # Collapse any candidates projected onto the same dense route sample.
        keep = np.r_[np.diff(candidate_station) > 1.0e-9, True]
        candidate_keep_indices = np.flatnonzero(keep)
        candidate_station = candidate_station[keep]
        candidate_offset = candidate_offset[keep]
        if len(candidate_station):
            path_start_progress = float(candidate_station[0])

    available = cumulative[-1] - (origin_s + path_start_progress)
    if available < path_length_m:
        raise ValueError(
            f"only {available:.2f}m of route remains after the actual model "
            f"start; {path_length_m:.2f}m is required"
        )

    # station is relative to the first published model point.  Route lookup
    # uses its real progress so the 0--s0 Local Route prefix is not published.
    station = np.arange(
        0.0,
        path_length_m + interval_m * 0.5,
        interval_m,
    ...
    route_progress = path_start_progress + station
    base_xy, _tangent = _interpolate_route(
        route,
        cumulative,
        origin_s + route_progress,
    )
    offset_xy = np.zeros_like(base_xy)
    if len(candidate_station):
        for axis in range(2):
            offset_xy[:, axis] = np.interp(
                route_progress,
                candidate_station,
                candidate_offset[:, axis],
                left=float(candidate_offset[0, axis]),
                right=float(candidate_offset[-1, axis]),
            )
        tail = route_progress > candidate_station[-1]
        if tail.any():
            tail_span = max(
                route_progress[-1] - candidate_station[-1], interval_m
            )
            u = np.clip(
                (route_progress[tail] - candidate_station[-1]) / tail_span,
                0.0,
                1.0,
            )
            smooth = 6.0 * u**5 - 15.0 * u**4 + 10.0 * u**3
            offset_xy[tail] *= (1.0 - smooth)[:, None]

    path_xy = base_xy + offset_xy
    derivative = np.gradient(path_xy, station, axis=0, edge_order=1)
    yaw = np.unwrap(np.arctan2(derivative[:, 1], derivative[:, 0]))
    curvature = np.gradient(yaw, station, edge_order=1)
    return {
        ...
        "yaw_rad": yaw.astype(np.float32),
        "curvature_per_m": curvature.astype(np.float32),
        "base_xy_m": base_xy.astype(np.float32),
        "path_start_progress_m": np.float32(path_start_progress),
        "candidate_station_m": (
            candidate_station - path_start_progress
        ).astype(np.float32),
        "candidate_keep_indices": candidate_keep_indices,
    }
