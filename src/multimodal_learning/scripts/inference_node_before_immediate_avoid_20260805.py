...
class InferenceNode:
    def __init__(self):
        ...
        rospy.loginfo(
            ...
            "lateral residual deadband %.2fm, action queue %d/%d, STOP %s",
    ...
    def select_runtime_action(self, probabilities):
        ...
        if self.enable_stop_action:
            ...
            # STOP remains available on the raw diagnostic topic, but runtime
            # routing is reduced to DRIVE versus AVOID. Renormalization is not
            # needed because comparing the two retained logits/probabilities
