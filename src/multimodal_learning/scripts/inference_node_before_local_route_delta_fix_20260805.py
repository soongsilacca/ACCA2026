...
class InferenceNode:
    def __init__(self):
        ...
        self.immediate_avoid_probability = float(
            rospy.get_param("~immediate_avoid_probability", 0.5)
        )
        ...
        self.refresh_avoid_plan = False
        self.latched_avoid_residual = None
        self.latched_avoid_candidate_speed = None
    ...
    def dynamic_callback(self, front_msg, left_msg, right_msg, odom_msg, imu_msg):
        ...
        with torch.no_grad(), torch.amp.autocast(
            ...
            if runtime_action == self.action_avoid:
                if (
                    self.refresh_avoid_plan
                    or self.latched_avoid_residual is None
                    or self.latched_avoid_candidate_speed is None
                ):
                    self.latched_avoid_residual = lateral_residual.copy()
                    self.latched_avoid_candidate_speed = candidate_speed.copy()
                    self.refresh_avoid_plan = False
                lateral_residual_for_path = self.latched_avoid_residual
                candidate_speed_for_path = self.latched_avoid_candidate_speed
            else:
                self.latched_avoid_residual = None
                self.latched_avoid_candidate_speed = None
                lateral_residual_for_path = lateral_residual
                candidate_speed_for_path = candidate_speed
            applied_residual = (
                lateral_residual_for_path
                if runtime_action == self.action_avoid
    ...
    def publish_path_markers(self, trajectory, stamp, runtime_action):
        ...
        action_label.text = self.action_names[runtime_action]
        action_label.lifetime = line.lifetime

        marker_array.markers.extend((line, points, action_label))
    ...
    def publish_prediction(self, _event):
        ...
        action_name = self.action_names[runtime_action]
        self.runtime_action_publisher.publish(String(data=action_name))
        if runtime_action != self.last_announced_runtime_action:
            previous_name = (
                "NONE"
                if self.last_announced_runtime_action is None
                else self.action_names[self.last_announced_runtime_action]
            )
            rospy.logwarn(
            ...
            self.last_announced_runtime_action = runtime_action
