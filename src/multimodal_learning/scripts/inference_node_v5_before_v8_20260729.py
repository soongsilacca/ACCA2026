...
class InferenceNode:
    def __init__(self):
        ...
        model_source_dir = rospy.get_param(
            "~model_source_dir",
            "/media/acca/UBUNTU 20_0",
        )
        checkpoint_path = rospy.get_param(
            "~checkpoint_path",
            "/media/acca/UBUNTU 20_0/epoch_014.pt",
        )
        ...
        from trajectory_smoothing import smooth_positions
        ...
        self.model = MultiViewTemporalTrajectoryPlannerV5(
        ...
        rospy.loginfo(
            "Planner V5 epoch 14 loaded on %s: %d-frame trajectory, %.2fm spacing",
            self.device, self.history_size, self.path_spacing,
        )
    ...
    def map_callback(self, msg):
        ...
        try:
            ...
            rospy.logwarn_throttle(2.0, "Invalid V5 MGeo tokens: %s", error)
            return
        with self.static_lock:
            self.map_input = value
            self.static_revision += 1
        rospy.loginfo_once("Planner V5 MGeo input ready")
    ...
    def route_callback(self, msg):
        ...
        try:
            ...
            rospy.logwarn_throttle(2.0, "Invalid V5 route: %s", error)
            return
        with self.static_lock:
            self.route_input, self.route_msg = value, msg
            self.static_revision += 1
        rospy.loginfo_once("Planner V5 local route input ready")

    def tensor(self, value):
        return torch.from_numpy(value).unsqueeze(0).to(self.device)

    def smooth_absolute_prediction(self, prediction):
        """Smooth absolute local x/y and resample at fixed metric spacing."""
        ...
        source_distance = np.concatenate(([0.0], np.cumsum(segment_length)))
        keep = np.concatenate(([True], np.diff(source_distance) > 1e-6))
        source_distance = source_distance[keep]
        smoothed = smoothed[keep]

        if len(source_distance) < 2:
            return (
                raw_positions[:1].astype(np.float32),
                np.zeros(1, dtype=np.float32),
                np.asarray([prediction[0, 3]], dtype=np.float32),
            )

        total_distance = float(source_distance[-1])
        target_distance = np.arange(
            0.0, total_distance, self.path_spacing, dtype=np.float64
        )
        if len(target_distance) == 0 or not np.isclose(
            target_distance[-1], total_distance
        ):
            target_distance = np.append(target_distance, total_distance)

        positions = np.column_stack((
            np.interp(target_distance, source_distance, smoothed[:, 0]),
            np.interp(target_distance, source_distance, smoothed[:, 1]),
        ))
        ...
        speeds = np.interp(
            target_distance, source_distance, source_speed
        )
        source_yaw = np.unwrap(np.concatenate((
            [0.0],
            np.asarray(prediction[:, 2], dtype=np.float64),
        )))[keep]
        yaws = np.interp(
            target_distance, source_distance, source_yaw
        )
        return (
    ...
    def dynamic_callback(self, front_msg, left_msg, right_msg, odom_msg, imu_msg):
        rospy.loginfo_once("Planner V5 synchronized sensor callback active")
        ...
        if lidar_msg is None:
            rospy.logerr_throttle(
                2.0,
                "Planner V5 waiting for required LiDAR topic %s; "
        ...
        if lidar_stamp <= 0.0 or abs(stamp - lidar_stamp) > self.lidar_sync_tolerance:
            rospy.logerr_throttle(
                2.0,
                "Planner V5 LiDAR is stale/unsynchronized: age %.3fs "
        ...
        try:
            ...
            rospy.logwarn_throttle(2.0, "Invalid dynamic V5 input: %s", error)
            return
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            rospy.loginfo_once("Planner V5 history/static inputs ready")
            mgeo, route = static_cache
            output = self.model(*history, mgeo, route)
            rospy.loginfo_once("Planner V5 first forward completed")
            ...
            local_trajectory = np.column_stack((positions, yaws, speeds))
            ...
            cpu_output = {
                ...
                "mode_scores": np.asarray([1.0], dtype=np.float32),
            }
    ...
    def publish_prediction(self, _event):
        ...
        if (rospy.Time.now() - update_time).to_sec() > self.prediction_timeout:
            rospy.logwarn_throttle(2.0, "Planner V5 prediction stale; publication paused")
            return
