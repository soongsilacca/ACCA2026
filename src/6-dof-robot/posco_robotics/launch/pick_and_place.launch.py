...
def generate_launch_description():
    ...
    # Gripper Grasp Controller (for automatic grasping)
    gripper_grasp_controller = Node(
        package='posco_robotics',
        executable='gripper_grasp_controller.py',
        name='gripper_grasp_controller',
        output='screen',
        parameters=[{'use_sim_time': True}]
    )
    ...
    # Add grasp plugin to Gazebo plugin path
    gazebo_plugin_path = os.path.join(get_package_prefix('gazebo_grasp_plugin'), 'lib')
    
    if 'GAZEBO_PLUGIN_PATH' in os.environ:
        gazebo_plugin_path = os.environ['GAZEBO_PLUGIN_PATH'] + ':' + gazebo_plugin_path
        
    gazebo_plugin_env = SetEnvironmentVariable("GAZEBO_PLUGIN_PATH", gazebo_plugin_path)
    
    print(f"DEBUG: GAZEBO_PLUGIN_PATH set to: {gazebo_plugin_path}")
    ...
    gazebo = IncludeLaunchDescription(
        ...
        launch_arguments={
            'world': os.path.join(pkg_posco_robotics, 'worlds', 'pick_and_place.world'),
            'verbose': 'true'
        }.items()
    )
