...
class CollisionEStop(object):
    ...
    def __init__(self):
        ...
        self.route_reach_seed = None    # previous match, windows the next search
        self.route_reach_window = rospy.get_param("~route_reach_window", 8.0)
        self.route_side_margin = rospy.get_param("~route_side_margin", 8.0)

        # --- lateral approach tracking ------------------------------------------
        # Watches a band just outside the corridor and measures how fast the
        # nearest return in each longitudinal bin is closing on the path.
        # Route jitter shows up here as apparent lateral motion, so the
        # threshold is deliberately loose and a rate has to persist for
        # several frames before it counts -- jitter is zero-mean and does not
        # survive that, a real cut-in does.
        self.lateral_enable = rospy.get_param("~lateral_enable", True)
        self.watch_band = rospy.get_param("~watch_band", 2.5)
        self.lat_bin_size = rospy.get_param("~lat_bin_size", 5.0)
        # Set above the observed route-jitter band, not from vehicle dynamics.
        self.lat_rate_min = rospy.get_param("~lat_rate_min", 1.0)
        self.lat_rate_alpha = rospy.get_param("~lat_rate_alpha", 0.3)
        self.lat_confirm_bins = rospy.get_param("~lat_confirm_bins", 3)
        self.lat_range_ratio = rospy.get_param("~lat_range_ratio", 0.6)
        self.lat_bins = {}
        self.incursion_point = None
        self.lat_bin_time = None
        self.incursion_ttc = SAFE_TTC
        self.incursion_point = None     # (x, y, closing rate) of the flagged bin
        ...
        self.obs_point = None           # (x, y) of the matched obstacle, base_link
        ...
        self.pub_inc = rospy.Publisher("/safety/incursion_ttc", Float32, queue_size=1)
    ...
    def lidar_cb(self, msg):
        ...
        if route_ok:
            d, obs_xy = self.nearest_along_route(p, route_xy, watch_range)
            source = "route"
        else:
            d, obs_xy = self.nearest_in_corridor(p, v, w, watch_range)
            source = "arc"

        inc, inc_point = SAFE_TTC, None
        if self.lateral_enable:
            inc, inc_point = self.track_lateral_approach(
                p, v, w, watch_range, stamp.to_sec(),
                route_xy if route_ok else None)
        ...
        with self.lock:
            ...
            self.d_obs = d
            self.obs_point = obs_xy
            self.incursion_ttc = inc
            self.incursion_point = inc_point
    ...
    def nearest_along_route(self, p, route_xy, watch_range):
        ...
        Returns (reach, point_xy) so the caller can mark exactly what was
        detected, or (inf, None) when nothing matched.

        Reach search is windowed around the previous frame's match. Matching
        against the full route lets a point get attached to a distant segment
        that happens to curve close to it in space (loops, tight intersection
        turns) even though the path to reach it is long -- a stationary
        object near the car then reads as "on the path 60 m ahead". Seeding
        the search from the last known reach keeps the match temporally
        continuous and rejects this kind of long-range wraparound; the window
        is dropped after a miss so a genuinely new object is still found.
        """
        if p.size == 0 or route_xy.shape[0] < 2:
            return float("inf"), None

        half_w = self.half_width()
        route_xy = self.clip_route(route_xy, watch_range)
        side = self.route_side_limit(route_xy)
        keep = ((p[:, 2] > self.ground_z) & (p[:, 2] < self.ceiling_z) &
                (p[:, 0] > -1.0) & (p[:, 0] < watch_range + 5.0) &
                (np.abs(p[:, 1]) < side))
        q = p[keep]
        ...
        if self.route_reach_seed is not None:
            lo = max(0.0, self.route_reach_seed - self.route_reach_window)
            hi = self.route_reach_seed + self.route_reach_window
            seg_idx = np.where((cum_len[:-1] < hi) & (cum_len[1:] > lo))[0]
            if seg_idx.size == 0:
                seg_idx = np.arange(seg_start.shape[0])
        else:
            seg_idx = np.arange(seg_start.shape[0])

        best_reach = float("inf")
        best_point = None
        # Vectorised over points, looped over the (windowed) segment set: the
        # route is a few dozen points at most, points are already reduced to
        # the watch box above, so this stays well within the 20 Hz budget.
        for i in seg_idx:
            rel = q[:, :2] - seg_start[i]
            ...
            if reach.size:
                ...
                if reach[j] < best_reach:
                    best_reach = float(reach[j])
                    best_point = tuple(q[inside][j, :2])

        if not math.isfinite(best_reach):
            self.route_reach_seed = None
            return float("inf"), None

        self.route_reach_seed = best_reach
        return max(0.0, best_reach - self.front_bumper), best_point

    # ---------------------------------------------------------------- arc corridor

    def nearest_in_corridor(self, p, v, yaw_rate, watch_range):
        """Along-path distance and matched point for an arc derived from yaw
        rate. Fallback when the route is stale, short, or non-finite.

        Returns (reach, point_xy) or (inf, None).
        """
        if p.size == 0:
            return float("inf"), None
        ...
        if abs(yaw_rate) < 0.01 or abs(v) < 0.5:
            ...
            if np.count_nonzero(inside) < self.min_points:
                return float("inf"), None
            cand = p[inside]
            j = int(np.argmin(cand[:, 0]))
            reach = float(cand[j, 0])
            point = (float(cand[j, 0]), float(cand[j, 1]))
        else:
            radius = v / yaw_rate
            ...
            reach = float(abs(radius) * theta[j])
            point = (float(q[j, 0]), float(q[j, 1]))

        return max(0.0, reach - self.front_bumper), point

    def clip_route(self, route_xy, reach_limit):
        """Trim the route to the first `reach_limit` metres of arc length.

        Segments beyond the watch range cannot produce a valid detection, but
        they still cost a full pass over every candidate point in the segment
        loop -- on a 130-point route that is most of the work for none of the
        result.
        """
        if route_xy.shape[0] < 3:
            return route_xy
        seg = np.hypot(np.diff(route_xy[:, 0]), np.diff(route_xy[:, 1]))
        cum = np.concatenate([[0.0], np.cumsum(seg)])
        n = int(np.searchsorted(cum, reach_limit)) + 2
        return route_xy[:max(3, min(n, route_xy.shape[0]))]

    def route_side_limit(self, route_xy):
        """Lateral half-extent worth considering, from the route's own spread.

        A point further out than this cannot fall inside the corridor or the
        approach band no matter which segment it is tested against, so it is
        dropped before the expensive per-segment pass. Derived from the route
        rather than fixed, so a route that swings wide on a curve widens the
        cut with it.
        """
        span = float(np.max(np.abs(route_xy[:, 1]))) if route_xy.size else 0.0
        return span + self.half_width() + self.watch_band + self.route_side_margin

    def route_offsets(self, q, route_xy):
        """Per-point (lateral offset, along-route distance) against the route.

        Offset is the distance to the closest point on the polyline, so it is
        the route-relative equivalent of the arc corridor's lateral gap.
        """
        seg_start = route_xy[:-1]
        seg_vec = route_xy[1:] - seg_start
        seg_len2 = np.sum(seg_vec ** 2, axis=1)
        ...
        best_off = np.full(q.shape[0], np.inf)
        best_along = np.zeros(q.shape[0])
        for i in range(seg_start.shape[0]):
            ...
            proj = seg_start[i] + t[:, None] * seg_vec[i]
            lateral = np.hypot(q[:, 0] - proj[:, 0], q[:, 1] - proj[:, 1])
            closer = lateral < best_off
            best_off = np.where(closer, lateral, best_off)
            best_along = np.where(closer,
                                  cum_len[i] + t * np.sqrt(seg_len2[i]),
                                  best_along)
        return best_off, best_along

    # ------------------------------------------------- lateral approach tracking

    def track_lateral_approach(self, p, v, yaw_rate, watch_range, stamp,
                               route_xy=None):
        """Time until the fastest laterally-closing return reaches the corridor.

        Offsets are measured against the local route when one is available,
        falling back to the yaw-rate arc otherwise.

        The route jitters frame to frame, and at ~7 Hz even 0.2 m of jitter
        reads as 1.5 m/s of apparent closing speed. Two things separate that
        from a real cut-in. Jitter is zero-mean, so a heavily smoothed rate
        averages it away while a sustained one-directional approach survives;
        and a real approach persists, so a rate has to stay above threshold
        for several frames before it counts. The threshold itself sits above
        the observed jitter band rather than at a physically motivated value.
        """
        half_w = self.half_width()
        outer = half_w + self.watch_band

        # Cut-ins resolve within a few tens of metres, and returns further out
        # are too sparse for a per-bin offset to be stable, so this tracker
        # works over a shorter range than the in-corridor test.
        lat_range = watch_range * self.lat_range_ratio
        if route_xy is not None and route_xy.shape[0] >= 2:
            route_xy = self.clip_route(route_xy, lat_range)
            side = self.route_side_limit(route_xy)
        else:
            side = half_w + self.watch_band + self.route_side_margin

        keep = ((p[:, 2] > self.ground_z) & (p[:, 2] < self.ceiling_z) &
                (p[:, 0] > 0.0) & (p[:, 0] < lat_range) &
                (np.abs(p[:, 1]) < side))
        q = p[keep]
        if q.shape[0] < self.min_points:
            self.lat_bins, self.lat_bin_time = {}, stamp
            return SAFE_TTC, None

        if route_xy is not None and route_xy.shape[0] >= 2:
            offset, along = self.route_offsets(q, route_xy)
        elif abs(yaw_rate) < 0.01 or abs(v) < 0.5:
            offset, along = np.abs(q[:, 1]), q[:, 0]
        else:
            radius = v / yaw_rate
            offset = np.abs(np.hypot(q[:, 0], q[:, 1] - radius) - abs(radius))
            theta = np.arctan2(q[:, 0], np.sign(radius) * (radius - q[:, 1]))
            theta = np.where(theta < 0.0, theta + 2.0 * math.pi, theta)
            along = abs(radius) * theta

        band = (offset >= half_w) & (offset < outer) & (along > 0.0)
        dt = (stamp - self.lat_bin_time) if self.lat_bin_time is not None else 0.0
        self.lat_bin_time = stamp

        raw_bins = {}
        if np.any(band):
            keys = np.floor(along[band] / self.lat_bin_size).astype(np.int32)
            offs = offset[band]
            pts = q[band]
            for k in np.unique(keys):
                sel = keys == k
                j = int(np.argmin(offs[sel]))
                raw_bins[int(k)] = (float(offs[sel][j]),
                                    float(pts[sel][j, 0]), float(pts[sel][j, 1]))

        best = SAFE_TTC
        best_point = None
        updated = {}
        for k, (off_now, px, py) in raw_bins.items():
            prev = self.lat_bins.get(k)
            if prev is None or not (1e-3 < dt < 1.0):
                updated[k] = (off_now, 0.0, 0)
                continue
            prev_off, prev_rate, prev_hits = prev
            rate = ((prev_off - off_now) / dt) * self.lat_rate_alpha \
                + prev_rate * (1.0 - self.lat_rate_alpha)
            hits = prev_hits + 1 if rate >= self.lat_rate_min else 0
            updated[k] = (off_now, rate, hits)
            if hits >= self.lat_confirm_bins and rate > 1e-3:
                t_cross = (off_now - half_w) / rate
                if 0.0 <= t_cross < best:
                    best = t_cross
                    best_point = (px, py, rate)

        # bins not refreshed this frame are dropped, so an object leaving the
        # band cannot keep a stale rate alive
        self.lat_bins = updated
    ...
    def update(self, _event):
        ...
        #     return self.emit(SAFE_TTC, d, inc)
        ...
        # A return outside the corridor but closing on it fast enough to cross
        # within the E-STOP horizon. Additive only: it can raise a stop the
        # in-corridor tests would have raised late, never suppress one.
        cond_incursion = inc < self.ttc_estop

        if cond_ttc or cond_brake or cond_incursion:
            self.danger_count += 1
            self.safe_since = None
            if self.danger_count >= self.confirm_frames:
                if cond_ttc:
                    self.latch("TTC_%.2fs" % ttc)
                elif cond_brake:
                    self.latch("BRAKE_DIST_%.1fm" % d)
                else:
                    self.latch("INCURSION_%.2fs" % inc)
        else:
            self.danger_count = 0
            released = (ttc > self.ttc_release and inc > self.ttc_release and
    ...
    def emit(self, ttc, d, inc=SAFE_TTC):
        with self.lock:
        ...
        self.pub_inc.publish(Float32(data=min(inc, SAFE_TTC)))

    # ------------------------------------------------------------ visualisation

    def draw_markers(self, v):
        with self.lock:
            w = self.yaw_rate
            ...
            obs_point = self.obs_point
            inc_point = self.incursion_point
            estop = self.estop
            reason = self.reason
        ...
        band = Marker()
        band.header.frame_id = "base_link"
        band.header.stamp = rospy.Time.now()
        band.ns, band.id = "safety_corridor", 0
        band.type, band.action = Marker.LINE_STRIP, Marker.ADD
        band.pose.orientation.w = 1.0
        band.scale.x = self.ego_width + 2.0 * self.lateral_margin
        band.color.a = 0.3
        if estop:
            band.color.r = 1.0
        else:
            band.color.g, band.color.b = 0.5, 1.0
        band.lifetime = rospy.Duration(0.3)

        if source == "route" and route_xy is not None and route_xy.shape[0] >= 2:
            cum = 0.0
            band.points.append(self._pt(route_xy[0, 0], route_xy[0, 1]))
            for i in range(1, route_xy.shape[0]):
                ...
                if cum > rng:
                    break
                band.points.append(self._pt(route_xy[i, 0], route_xy[i, 1]))
        else:
            speed = max(abs(v), 1.0)
            for i in range(21):
                s = self.front_bumper + (rng - self.front_bumper) * i / 20.0
                if abs(w) < 0.01:
                    band.points.append(self._pt(s, 0.0))
                else:
                    radius = speed / w
                    theta = s / abs(radius)
                    band.points.append(self._pt(abs(radius) * math.sin(theta),
                                                radius * (1.0 - math.cos(theta))))
        arr.markers.append(band)

        # Detected obstacle. Two kinds, kept visually distinct because they
        # mean different things: a red sphere is something already in the path
        # and closing head-on, an orange cube is something beside the path
        # moving into it, with an arrow showing the closing direction.
        cause = reason.split("|")[0]
        if estop and not cause.endswith("STALE"):
            if cause.startswith("INCURSION") and inc_point is not None:
                ix, iy, irate = inc_point
                cube = Marker()
                cube.header.frame_id = "base_link"
                cube.header.stamp = rospy.Time.now()
                cube.ns, cube.id = "safety_incursion", 3
                cube.type, cube.action = Marker.CUBE, Marker.ADD
                cube.pose.position.x, cube.pose.position.y = ix, iy
                cube.pose.position.z = 0.5
                cube.pose.orientation.w = 1.0
                cube.scale.x = cube.scale.y = cube.scale.z = 0.8
                cube.color.r, cube.color.g, cube.color.a = 1.0, 0.55, 0.9
                cube.lifetime = rospy.Duration(0.3)
                arr.markers.append(cube)

                # arrow from the object toward the path, length scaled by how
                # fast it is closing
                arrow = Marker()
                arrow.header.frame_id = "base_link"
                arrow.header.stamp = rospy.Time.now()
                arrow.ns, arrow.id = "safety_incursion_dir", 4
                arrow.type, arrow.action = Marker.ARROW, Marker.ADD
                arrow.pose.orientation.w = 1.0
                arrow.scale.x, arrow.scale.y, arrow.scale.z = 0.12, 0.28, 0.3
                arrow.color.r, arrow.color.g, arrow.color.a = 1.0, 0.55, 0.9
                arrow.lifetime = rospy.Duration(0.3)
                arrow.points.append(self._pt(ix, iy, 0.5))
                arrow.points.append(self._pt(ix, iy - math.copysign(irate, iy), 0.5))
                arr.markers.append(arrow)

                arr.markers.append(self._label("safety_incursion_label", 5, ix, iy,
                                               "%s  %.1f m/s" % (cause, irate)))
            elif obs_point is not None:
                ox, oy = obs_point
                sphere = Marker()
                sphere.header.frame_id = "base_link"
                sphere.header.stamp = rospy.Time.now()
                sphere.ns, sphere.id = "safety_obstacle", 1
                sphere.type, sphere.action = Marker.SPHERE, Marker.ADD
                sphere.pose.position.x, sphere.pose.position.y = ox, oy
                sphere.pose.position.z = 0.5
                sphere.pose.orientation.w = 1.0
                sphere.scale.x = sphere.scale.y = sphere.scale.z = 0.6
                sphere.color.r, sphere.color.a = 1.0, 0.9
                sphere.lifetime = rospy.Duration(0.3)
                arr.markers.append(sphere)

                arr.markers.append(self._label("safety_obstacle_label", 2,
                                               ox, oy, cause))

        self.pub_marker.publish(arr)

    def _label(self, ns, mid, x, y, text):
        m = Marker()
        m.header.frame_id = "base_link"
        m.header.stamp = rospy.Time.now()
        m.ns, m.id = ns, mid
        m.type, m.action = Marker.TEXT_VIEW_FACING, Marker.ADD
        m.pose.position.x, m.pose.position.y, m.pose.position.z = x, y, 1.3
