...
class MGeoRoutePublisher:
    def __init__(self):
        ...
        self.max_lateral_accel = float(
            rospy.get_param("~max_lateral_accel", 3.0)
        )
        self.max_longitudinal_decel = float(
            rospy.get_param("~max_longitudinal_decel", 2.0)
        )
        ...
        if self.max_longitudinal_decel <= 0.0:
            raise ValueError("max_longitudinal_decel must be positive")
    ...
    def build_speed_profile(self):
        """Combine MGeo link limits with global-path curvature limits."""
        tree = cKDTree(self.map_points)
        _distance, nearest = tree.query(self.global_path, k=1)
        link_speed = self.map_attributes[nearest, 6]
        profile = np.where(
            np.isfinite(link_speed) & (link_speed > 0.0),
            link_speed,
            self.default_speed_mps,
        )
        profile = np.minimum(profile, self.speed_cap_mps)

        closed = (
            len(self.global_path) > 2
            and np.linalg.norm(
                self.global_path[0] - self.global_path[-1]
            ) < 1.0e-4
        )
        unique_path = self.global_path[:-1] if closed else self.global_path
        if closed:
            incoming = unique_path - np.roll(unique_path, 1, axis=0)
            outgoing = np.roll(unique_path, -1, axis=0) - unique_path
            cross = (
            ...
            curvature = np.abs(
                ...
                / np.maximum(local_length, 1.0e-4)
            )
        else:
            dx = np.gradient(unique_path[:, 0])
            dy = np.gradient(unique_path[:, 1])
            ddx = np.gradient(dx)
            ddy = np.gradient(dy)
            curvature = np.abs(
                (dx * ddy - dy * ddx)
                / np.maximum((dx * dx + dy * dy) ** 1.5, 1.0e-8)
            )

        path_spacing = float(np.median(
            np.linalg.norm(np.diff(unique_path, axis=0), axis=1)
        ))
        if self.curvature_smoothing_m > 0.0:
            sigma = self.curvature_smoothing_m / max(path_spacing, 1.0e-4)
        ...
        if closed:
            curvature = np.concatenate((curvature, curvature[:1]))

        curve_speed = np.sqrt(
            self.max_lateral_accel / np.maximum(curvature, 1.0e-5)
        )
        profile = np.minimum(profile, curve_speed)

        # Begin reducing the spatial target before each curve using
        # v_before^2 <= v_curve^2 + 2*a*distance.
        ...
        rospy.loginfo(
            "MGeo curvature-limited speed profile ready: %.1f-%.1f km/h, "
            "a_lat=%.1f m/s^2, decel=%.1f m/s^2, %d points",
    ...
    def publish_target_speed(self):
        if len(self.global_path_speed) == 0:
            return
        target = float(self.global_path_speed[self.last_path_index])
        self.speed_pub.publish(Float32(data=target))
        rospy.loginfo_throttle(
            1.0,
            "MGeo curvature-limited target speed: %.2f m/s (%.1f km/h)",
            target,
            target * 3.6,
        )
