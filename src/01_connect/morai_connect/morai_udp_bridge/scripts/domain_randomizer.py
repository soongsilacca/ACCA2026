...
def parse_ros_args():
    input_path = os.path.expanduser("~/MoraiLauncher_Lin/MoraiLauncher_Lin_Data/SaveFile/Scenario/R_KR_PR_K-city_2025/2026_molit_comp_sample_scene.json")
    output_path = os.path.expanduser("~/MoraiLauncher_Lin/MoraiLauncher_Lin_Data/SaveFile/Scenario/R_KR_PR_K-city_2025/randomized_final.json")
    count = 1
...
def main():
    # roslaunch 로 실행될 때 ROS private param(~), CLI 실행 시 argparse 사용
    is_ros_node = any(arg.startswith('__name:=') or arg.startswith('__log:=') for arg in sys.argv)

    if is_ros_node:
        import rospy
        if not rospy.core.is_initialized():
            rospy.init_node('domain_randomizer', anonymous=False)
        default_input = os.path.expanduser("~/MoraiLauncher_Lin/MoraiLauncher_Lin_Data/SaveFile/Scenario/R_KR_PR_K-city_2025/2026_molit_comp_sample_scene.json")
        default_output = os.path.expanduser("~/MoraiLauncher_Lin/MoraiLauncher_Lin_Data/SaveFile/Scenario/R_KR_PR_K-city_2025/randomized_final.json")
        input_path  = rospy.get_param('~input',  default_input)
        output_path = rospy.get_param('~output', default_output)
