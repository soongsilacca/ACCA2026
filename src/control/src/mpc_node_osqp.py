...
class MPCController:
    def __init__(self):
        ...
        # CVXPY + OSQP 워밍스타트용 이전 스텝 해 캐시 (없으면 콜드스타트)
        self._cvxpy_prev_x = None
        self._cvxpy_prev_u = None

        self.solve_times = []
        self.solve_time_csv_path = rospy.get_param(
                '~solve_time_csv_path',
                '/home/acca/tracking_logs/mpc_osqp_solve_times.csv'
            )
    ...
    def solve_mpc(self, x0, A_list, B_list, b_list, Q_list, R_list, q_list, r_list):
        """CVXPY 및 OSQP 솔버를 사용하여 선형 MPC QP 최적화 문제를 풉니다."""
        T = self.T
        nx = self.nx
        nu = self.nu

        x_var = cp.Variable((nx, T + 1))
        u_var = cp.Variable((nu, T))
        ...
        for t in range(T):
            constraints.append(
                x_var[:, t + 1] == A_list[t] @ x_var[:, t] + B_list[t] @ u_var[:, t] + b_list[t]
            )
            constraints.append(x_var[:, t + 1] >= self.lbx)
            constraints.append(x_var[:, t + 1] <= self.ubx)
            constraints.append(u_var[:, t] >= self.lbu)
            constraints.append(u_var[:, t] <= self.ubu)

            cost += 0.5 * cp.quad_form(x_var[:, t], Q_list[t]) + q_list[t] @ x_var[:, t]
            cost += 0.5 * cp.quad_form(u_var[:, t], R_list[t]) + r_list[t] @ u_var[:, t]

        cost += 0.5 * cp.quad_form(x_var[:, T], Q_list[T]) + q_list[T] @ x_var[:, T]

        prob = cp.Problem(cp.Minimize(cost), constraints)

        if getattr(self, '_cvxpy_prev_x', None) is not None and getattr(self, '_cvxpy_prev_u', None) is not None:
            if self._cvxpy_prev_x.shape == (nx, T + 1) and self._cvxpy_prev_u.shape == (nu, T):
                x_var.value = self._cvxpy_prev_x
                u_var.value = self._cvxpy_prev_u
        ...
        try:
            ...
            status_ok = prob.status in (cp.OPTIMAL, cp.OPTIMAL_INACCURATE)
        except Exception as e:
            rospy.logerr_throttle(1.0, f"CVXPY solve exception: {e}")
            status_ok = False

        if status_ok and u_var.value is not None:
            self._cvxpy_prev_x = x_var.value.copy()
            self._cvxpy_prev_u = u_var.value.copy()

            raw_u_seq = u_var.value.T.astype(float)
        ...
        rospy.logwarn_throttle(
            1.0,
            "OSQP solve failed (status=%s). Stanley fallback is active.",
            prob.status
        )
