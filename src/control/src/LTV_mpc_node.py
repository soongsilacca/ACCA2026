...
class MPCController:
    def __init__(self):
        ...
        self.v_error_integral = 0
        self.prev_accel_cmd = 0
      
        self.Q = np.diag([self.q_y, self.q_v, self.q_yaw])
        self.Qf = np.diag([self.qf_y, self.qf_v, self.qf_yaw])
        ...
        self.prev_accel = np.zeros(self.T)
        self.prev_delta = np.zeros(self.T)
    ...
    def solve_mpc(self, x0, A_list, B_list, b_list, Q_list, R_list, q_list, r_list):
        ...
        if status == 0:
            ...
            u_seq = np.zeros((self.T, self.nu))

            for t in range(self.T):
                u_seq[t, :] = qp_sol.get('u', t)
            ...
            u0 = qp_sol.get('u', 0)
            rospy.loginfo(f"steer : {u0[1]}, accel : {u0[0]}")
            accel_cmd = u0[0]
            steer_cmd = u0[1]
    ...
    def get_state_space_matrices(self, v, e_yaw, delta):
        ...
        # e_ct(k+1)
        A[0,1] = math.sin(e_yaw) * self.dt
        A[0,2] = v * math.cos(e_yaw) * self.dt

        # e_yaw(k+1)
        A[2,1] = math.tan(delta) / self.wheelbase * self.dt

        # acceleration
        B[1,0] = self.dt

        # steering
        B[2,1] = (v / (self.wheelbase * math.cos(delta)**2)) * self.dt
    ...
    def predict_motion(self, x0, oa, odelta):
        ...
        for t in range(self.T):
            ...
            e_ct = e_ct + v * e_yaw * self.dt

            v = v + a * self.dt

            e_yaw = e_yaw + (
                v / self.wheelbase
            ) * math.tan(delta) * self.dt
    ...
    def prepare_mpc_matrices(self, xref):
        T = self.T 
        A_list, B_list, b_list = [], [], []
        Q_list, R_list, q_list, r_list = [], [], [], []

        indices = self.get_prediction_indices()
        ...
        for t in range(T):
            ...
            A_d, B_d = self.get_state_space_matrices(xbar[1, t], xbar[2,t], self.prev_delta[t])

            b_d = self.get_affine_term(xbar[1, t], kappa)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            oa = np.zeros(self.T)
            odelta = np.zeros(self.T)

            xbar = self.predict_motion(x0, self.prev_accel, self.prev_delta)                   

            xbar = np.zeros((self.nx, self.T + 1))
            for t in range(self.T + 1):
                xbar[:, t] = xref[:, t]  

            A_list, B_list, b_list, Q_list, R_list, q_list, r_list = self.prepare_mpc_matrices(xref)
            
            steer, accel, brake = self.solve_mpc(x0, A_list, B_list, b_list, Q_list, R_list, q_list, r_list)
