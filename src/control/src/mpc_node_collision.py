...
class MPCController:
    def __init__(self):
        ...
        self.controller_mode = str(
            rospy.get_param('~controller_mode', 'mpc')
        ).strip().lower()
        ...
        self.stanley_max_lateral_accel = float(rospy.get_param(
            '~stanley_max_lateral_acceleration_mps2', 3.0
        ))
        self.stanley_curve_preview_m = float(rospy.get_param(
            '~stanley_curve_preview_m', 10.0
        ))
        self.stanley_curve_min_speed = float(rospy.get_param(
            '~stanley_curve_min_speed_mps', 1.0
        ))
        if self.stanley_max_lateral_accel <= 0.0:
            raise ValueError("Stanley maximum lateral acceleration must be positive")
        if self.stanley_curve_preview_m <= 0.0:
            raise ValueError("Stanley curve preview distance must be positive")
        if self.stanley_curve_min_speed < 0.0:
            raise ValueError("Stanley curve minimum speed cannot be negative")
        ...
        self.solve_time_csv_path = rospy.get_param(
                        '~solve_time_csv_path',
                        '/home/acca/tracking_logs/mpc_solve_times.csv'
                    )

        rospy.on_shutdown(self.mpc_solve_time)
        ...
        self.stanley_curve_target_pub = rospy.Publisher(
            '/stanley/curvature_target_velocity', Float32, queue_size=1
        )
        ...
        # --- Safety Monitor E-STOP (collision_estop_node) -----------------
        # External non-learned safety layer. While the trigger is true the
        # controller must brake regardless of its own solution. Handled with
        # the same pattern as the model full-brake latch, but this one is not
        # latched here: engage/release policy lives in the safety node, the
        # controller just obeys the current trigger state.
        self.safety_estop_active = False
        self.last_safety_msg_time = None
        self.safety_estop_timeout = float(
            rospy.get_param('~safety_estop_timeout', 0.0)
        )  # 0 disables the watchdog; >0 treats trigger silence as E-STOP
        rospy.Subscriber(
            rospy.get_param('~safety_estop_topic', '/safety/estop_trigger'),
            Bool, self.safety_estop_callback, queue_size=1,
        )

        self.rate = rospy.Rate(10)
        rospy.loginfo(
            "Trajectory follower started in %s mode",
            self.controller_mode.upper(),
        )
    ...
    def odom_callback(self, msg):
        ...
        self.stanley_state.v = max(self.current_v, 0.0)
    ...
    def path_callback(self, msg):
        ...
        path_poses = msg.poses[self.path_start_index:]
        if len(path_poses) < 2:
            rospy.logwarn_throttle(
                2.0,
                "Path has %d poses after start index %d; at least 2 required",
                len(path_poses),
    ...
    def stanley_curvature_target_speed(self, base_target_speed):
        """Apply a continuous lateral-acceleration speed cap to Stanley."""
        base_target_speed = max(float(base_target_speed), 0.0)
        if (
            self.ckappa is None
            or len(self.ckappa) == 0
            or len(self.path_s) != len(self.ckappa)
        ):
            return base_target_speed, float('inf'), 0.0

        start = int(np.clip(self.closest_idx, 0, len(self.ckappa) - 1))
        end = self.distance_to_index(self.stanley_curve_preview_m)
        end = max(start, min(end, len(self.ckappa) - 1))
        curvature = np.abs(self.ckappa[start:end + 1])
        station = self.path_s[start:end + 1] - self.path_s[start]
        finite = np.isfinite(curvature) & np.isfinite(station)
        if not np.any(finite):
            return base_target_speed, float('inf'), 0.0

        curvature = curvature[finite]
        station = station[finite]
        decay_distance = max(self.stanley_curve_preview_m * 0.5, 0.1)
        weights = np.exp(-station / decay_distance)
        effective_curvature = float(np.sqrt(
            np.sum(weights * np.square(curvature)) / np.sum(weights)
        ))
        curve_speed_limit = math.sqrt(
            self.stanley_max_lateral_accel
            / max(effective_curvature, 1.0e-6)
        )
        # Curvature may slow a moving vehicle, but the TCP STOP decision is
        # the only model-side condition allowed to command a full stop.
        curve_speed_limit = max(
            curve_speed_limit, self.stanley_curve_min_speed
        )
        return (
            min(base_target_speed, curve_speed_limit),
            curve_speed_limit,
            effective_curvature,
        )
    ...
    def safety_estop_callback(self, msg):
        self.safety_estop_active = bool(msg.data)
        self.last_safety_msg_time = rospy.Time.now()

    def safety_stop_required(self):
        if self.safety_estop_active:
            return True
        if (self.safety_estop_timeout > 0.0
                and self.last_safety_msg_time is not None
                and (rospy.Time.now() - self.last_safety_msg_time).to_sec()
                > self.safety_estop_timeout):
            return True
        return False
    ...
    def run_stanley_controller(self, target_velocity):
        """Stanley lateral control with PI longitudinal pedal commands."""
        steer_target = float(self.stanley_solver.stan_control(
            self.stanley_state,
            self.cx,
            self.cy,
            self.cyaw,
        ...
        steer_change = self.max_steer_rate * self.dt
        steer = float(np.clip(
            steer_target,
            self.last_steer_cmd - steer_change,
            self.last_steer_cmd + steer_change,
        ))
        steer = float(np.clip(steer, -self.max_steer, self.max_steer))

        velocity_error = float(target_velocity) - self.current_v
        self.v_error_integral += velocity_error * self.dt
        self.v_error_integral = float(np.clip(
            self.v_error_integral, -2.0, 2.0
        ))
        requested_accel = (
            self.kp * velocity_error
            + self.ki * self.v_error_integral
        )
        accel_change = self.max_accel_rate * self.dt
        signed_accel = float(np.clip(
            requested_accel,
            self.last_accel_cmd - accel_change,
            self.last_accel_cmd + accel_change,
        ))
        signed_accel = float(np.clip(
            signed_accel, -self.max_accel, self.max_accel
        ))

        self.last_steer_cmd = steer
        self.last_accel_cmd = signed_accel
        self.prev_accel_cmd = signed_accel

        max_accel = max(float(self.max_accel), 1.0e-6)
        max_brake = max(
            float(getattr(self, 'max_brake', self.max_accel)), 1.0e-6
        )
        brake_deadband = -0.3
        if signed_accel >= 0.0:
            accel = float(np.clip(signed_accel / max_accel, 0.0, 1.0))
            brake = 0.0
        elif signed_accel > brake_deadband:
            accel = 0.0
            brake = 0.0
        else:
            accel = 0.0
            brake = float(np.clip(
                (brake_deadband - signed_accel) / max_brake,
                0.0,
                1.0,
            ))
        return steer, accel, brake
    ...
    def solve_mpc(self, x0, A_list, B_list, b_list, Q_list, R_list, q_list, r_list):
        ...
        if self.qp_sol is None:
            self.qp_sol = hpipm.hpipm_ocp_qp_sol(dim)
            arg.set('warm_start', 0)  # 첫 실행 시 Cold Start
        else:
        ...
        while not rospy.is_shutdown():
            ...
            if self.controller_mode == 'stanley':
                (
                    ...
                    curve_speed_limit,
                    effective_curvature,
                ) = self.stanley_curvature_target_speed(self.target_v)
                self.stanley_curve_target_pub.publish(
                    Float32(control_target_v)
                )
                steer, accel, brake = self.run_stanley_controller(
                    control_target_v
                )
            else:
            ...
            # Safety Monitor E-STOP has the highest priority: while active,
            # the just-computed command is discarded and the vehicle brakes
            # straight. Checked before the model latch so external safety
            # always wins.
            ...
            if self.controller_mode == 'mpc':
                ...
                rospy.loginfo_throttle(
                    1.0,
                    "Stanley control: v=%.2f MGeo=%.2f curve_target=%.2f "
                    "m/s (limit=%.2f, curvature=%.4f) steer=%.3f "
                    "accel=%.3f brake=%.3f",
                    self.current_v,
                    self.target_v,
                    control_target_v,
    ...
    def mpc_solve_time(self):
        if not self.solve_times:
            rospy.logwarn("저장할 MPC solve time 데이터가 없습니다.")
            return

        times = np.asarray(self.solve_times, dtype=float)

        mean_time = np.mean(times)
        max_time = np.max(times)
        min_time = np.min(times)

        rospy.loginfo('========== Solve Time ============')
        rospy.loginfo('Mean : %3f ms', np.mean(times))
        rospy.loginfo('Max : %3f ms', np.max(times))
        rospy.loginfo('Min : %3f ms', np.min(times))

        # MPC solve time을 CSV로 저장
        try:
            csv_dir = os.path.dirname(self.solve_time_csv_path)
            if csv_dir:
                os.makedirs(csv_dir, exist_ok=True)

            with open(
                self.solve_time_csv_path,
                'w',
                newline='',
                encoding='utf-8'
            ) as f:
                writer = csv.writer(f)
                writer.writerow(['iteration', 'mpc_solve_time_ms'])

                for i, solve_time in enumerate(times, start=1):
                    writer.writerow([i, solve_time])

                writer.writerow([])

                writer.writerow(['STATISTICS', 'Mean', 'Max', 'Min'])

                writer.writerow(['', f'{mean_time:.6f}', f'{max_time:.6f}', f'{min_time:.6f}'])

            rospy.loginfo(
                "[파일 저장 성공] MPC solve time CSV: %s (%d samples)",
                self.solve_time_csv_path,
                len(times)
            )

        except Exception as e:
            rospy.logerr("MPC solve time CSV 저장 실패: %s", e)
