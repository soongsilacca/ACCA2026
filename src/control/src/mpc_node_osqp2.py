...
class MPCController:
    def __init__(self):
        ...
        # OSQP 워밍스타트용 이전 스텝 해 캐시 (없으면 콜드스타트)
        self._osqp_prev_x = None
        self._osqp_prev_u = None

        self.solve_times = []
        self.solve_time_csv_path = rospy.get_param(
                '~solve_time_csv_path',
                '/home/acca/tracking_logs/mpc_osqp2_solve_times.csv'
            )
    ...
    def run_stanley_controller(self):
        ...
        velocity_error = self.target_v - self.current_v
    ...
    def _build_osqp_qp(self, x0, A_list, B_list, b_list, Q_list, R_list, q_list, r_list):
        """HPIPM의 조건부동역학 OCP QP를, OSQP가 요구하는
        0.5*z'Pz + q'z  s.t.  l <= Az <= u
        형태의 dense-stacked QP로 변환합니다.

        결정변수 z = [x_0, x_1, ..., x_T, u_0, ..., u_{T-1}] (모두 stacked).
        등식 제약: x_0 = x0 (초기상태), x_{t+1} = A_t x_t + B_t u_t + b_t (동역학)
        부등식 제약: t=1..T 상태 박스, t=0..T-1 입력 박스
        """
        T = self.T
        nx = self.nx
        nu = self.nu
        ...
        # ---- 비용: P(block diag), q ----
        P_blocks = Q_list + R_list  # length T+1 + T
        P = sp.block_diag(P_blocks, format='csc')
        q = np.concatenate(q_list + r_list)

        # ---- 등식 제약: 초기상태 고정 + 선형화된 동역학 ----
        n_eq = nx * (T + 1)
        Aeq = sp.lil_matrix((n_eq, nZ))
        leq = np.zeros(n_eq)
        ueq = np.zeros(n_eq)

        Aeq[0:nx, x_idx(0):x_idx(0) + nx] = np.eye(nx)
        leq[0:nx] = x0
        ueq[0:nx] = x0

        for t in range(T):
            row0 = nx * (t + 1)
            Aeq[row0:row0 + nx, x_idx(t + 1):x_idx(t + 1) + nx] = np.eye(nx)
            Aeq[row0:row0 + nx, x_idx(t):x_idx(t) + nx] = -A_list[t]
            Aeq[row0:row0 + nx, u_idx(t):u_idx(t) + nu] = -B_list[t]
            leq[row0:row0 + nx] = b_list[t]
            ueq[row0:row0 + nx] = b_list[t]

        # ---- 부등식(박스) 제약: 상태(t=1..T) + 입력(t=0..T-1) ----
        n_bx = nx * T
        n_bu = nu * T
        n_ineq = n_bx + n_bu
        Aineq = sp.lil_matrix((n_ineq, nZ))
        lineq = np.zeros(n_ineq)
        uineq = np.zeros(n_ineq)

        for t in range(1, T + 1):
            row0 = nx * (t - 1)
            Aineq[row0:row0 + nx, x_idx(t):x_idx(t) + nx] = np.eye(nx)
            lineq[row0:row0 + nx] = self.lbx
            uineq[row0:row0 + nx] = self.ubx

        for t in range(T):
            row0 = n_bx + nu * t
            Aineq[row0:row0 + nu, u_idx(t):u_idx(t) + nu] = np.eye(nu)
            lineq[row0:row0 + nu] = self.lbu
            uineq[row0:row0 + nu] = self.ubu

        Acon = sp.vstack([Aeq, Aineq], format='csc')
        l = np.concatenate([leq, lineq])
        u = np.concatenate([ueq, uineq])

        return P, q, Acon, l, u, nX, nU

    def solve_mpc(self, x0, A_list, B_list, b_list, Q_list, R_list, q_list, r_list):

        P, q, Acon, l, u, nX, nU = self._build_osqp_qp(
            x0, A_list, B_list, b_list, Q_list, R_list, q_list, r_list
        )
        ...
        # 이전 스텝의 해로 워밍스타트 (있는 경우)
        if getattr(self, '_osqp_prev_x', None) is not None and getattr(self, '_osqp_prev_u', None) is not None:
            if self._osqp_prev_x.size == nX and self._osqp_prev_u.size == nU:
                solver.warm_start(x=np.concatenate([self._osqp_prev_x, self._osqp_prev_u]))
        ...
        # OSQP: 1 = solved, 2 = solved inaccurate. 그 외는 실패로 취급.
        status_ok = status_val in (1, 2)

        if status_ok:
            z = result.x
            self._osqp_prev_x = z[:nX].copy()
            self._osqp_prev_u = z[nX:].copy()

            raw_u_seq = z[nX:].reshape(self.T, self.nu).astype(float)

            if not np.all(np.isfinite(raw_u_seq)):
                raise ValueError(
                    "OSQP returned non-finite input sequence: %s" % (raw_u_seq,)
                )
        ...
        rospy.logwarn_throttle(
            1.0,
            "OSQP solve failed (status=%s, status_val=%d). Stanley fallback is active.",
            result.info.status,
            status_val
        )
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.controller_mode == 'mpc':
                ...
                rospy.loginfo_throttle(
                    1.0,
                    "Stanley control: v=%.2f target=%.2f m/s "
                    "steer=%.3f accel=%.3f brake=%.3f",
                    self.current_v,
                    self.target_v,
                    steer,
                    accel,
