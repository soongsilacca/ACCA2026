...
class AdaptiveWeightNode:
    ...
    def updateWeight(self):
        ...
        # Keep the original smooth speed scheduling in the normal-speed range.
        speed_reference = max(
            float(getattr(self, 'speed_weight_reference_kph', 35.0)) / 3.6,
            0.1
        )
        speed_factor = np.tanh(v / speed_reference)
        speed_kph = v * 3.6

        # Additional absolute-steering penalty activates smoothly around
        # 45 km/h. A narrow sigmoid leaves <=40 km/h nearly unchanged without
        # introducing a hard high-speed mode switch.
        high_speed_center = float(
            getattr(self, 'steer_high_speed_center_kph', 45.0)
        )
        high_speed_transition = max(
            float(getattr(self, 'steer_high_speed_transition_kph', 1.5)),
            0.1
        )
        high_speed_activation = 1.0 / (
            1.0 + np.exp(
                -(speed_kph - high_speed_center) / high_speed_transition
            )
        )
        ...
        gain_r = (
                ...
                + float(getattr(self, 'k_high_speed_r', 0.0))
