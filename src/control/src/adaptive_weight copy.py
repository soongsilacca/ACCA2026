...
class AdaptiveWeightNode:
    ...
    def updateWeight(self):
        ...
        v_norm = np.clip(v / self.max_speed, 0.0, 1.0)
        ...
        gain_ct = (
                ...
                + self.k_speed_q * np.tanh(v_norm)
            )

        Qct_target = self.q_ct_base * gain_ct
        ...
        Qyaw_target = self.q_yaw_base * gain_yaw

        gain_r = (
                1.0
                + self.k_steer * np.tanh(steer_rate_rms)
                + self.k_speed_r * np.tanh(v_norm)
