...
class MPCController:
    ...
    def odom_callback(self, msg):
        ...
        self.current_x = current_x + ((self.wheelbase * 3) / 4) * math.cos(self.current_yaw)
        self.current_y = current_y + ((self.wheelbase * 3) / 4) * math.sin(self.current_yaw)
    ...
    def update_curve_gain(self):
        ...
        end_idx = min(self.closest_idx + self.future, len(self.ckappa))
    ...
    def solve_mpc(self, x0, A_list, B_list, b_list, Q_list, R_list, q_list, r_list):
            ...
            if accel_cmd >= 0.0:
                ...
                if accel_cmd > brake_deadband:
                    ...
                    max_brake_command = float(
                        getattr(self, 'max_brake_command', 1.0)
                    )
                    brake = float(np.clip(
                        brake_ratio, 0.0, max_brake_command
                    ))
        ...
        if signed_accel >= 0.0:
            ...
            brake = min(
                brake,
                float(getattr(self, 'max_brake_command', 1.0))
            )
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # The input-rate limiter can retain a negative acceleration command
            # after the vehicle has already reached the target speed. Continuing
            # to apply that stale brake command causes a low-speed overshoot all
            # the way to standstill. Release the brake near the target and clear
            # only the remaining deceleration history.
            brake_release_margin = float(
                getattr(self, 'brake_release_margin', 0.3)
            )
            if (
                brake > 0.0
                and self.current_v <= self.target_v + brake_release_margin
            ):
                brake = 0.0
                self.last_accel_cmd = 0.0
                self.prev_accel = np.maximum(self.prev_accel, 0.0)
                rospy.loginfo_throttle(
                    1.0,
                    "Brake released near target: current=%.2f target=%.2f m/s",
                    self.current_v,
                    self.target_v,
                )
            ...
            rospy.loginfo_throttle(0.1, f'Q 행렬 : {self.Q}, R 행렬 {self.R}')
            self.rate.sleep()
