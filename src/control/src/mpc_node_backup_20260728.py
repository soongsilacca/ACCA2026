...
class MPCController:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            max_brake = float(getattr(self, 'max_brake', self.max_accel))
            ctrl_msg.accel = float(np.clip(accel / self.max_accel, 0.0, 1.0))
            ctrl_msg.brake = float(np.clip(brake / max_brake, 0.0, 1.0))
