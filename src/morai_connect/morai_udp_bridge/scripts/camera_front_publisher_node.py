#!/usr/bin/env python3
"""Camera (front) → sensor_msgs/Image  |  UDP 9291"""
import sys
...
def main():
    rospy.init_node('morai_camera_front_publisher', anonymous=False)
    ...
    rospy.loginfo(f"[Camera Front] UDP {IP}:{port} → {topic} (event-driven)")
    try:
