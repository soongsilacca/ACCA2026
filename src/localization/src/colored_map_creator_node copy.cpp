...
// FastGICP for stable scan-to-map registration
...
public:
    ColoredMapCreator() : nh_("~"), tf_buffer_(ros::Duration(1000.0)) {
        ...
        nh_.param<std::string>("save_dir",       save_dir_,       "/home/jinju/ros1_ws/src/localization/pcd");
        ...
        gicp_.setNumThreads(gicp_threads_);
    ...
    void process() {
        ...
        for (const rosbag::MessageInstance& m : lidar_view) {
            ...
            if (use_color_) {
                if (image_map.empty()) { ROS_WARN_THROTTLE(5, "No images."); continue; }
                ...
                if (std::abs((closest_time - lidar_time).toSec()) > time_sync_threshold_) continue;
            }
        ...
        // ── Final save ───────────────────────────────────────────
        if (global_map_->empty() && global_map_intensity_->empty()) {
            ROS_WARN("Both maps are empty. Nothing saved.");
            return;
        }
...
private:
    ...
    void processFrame(const sensor_msgs::PointCloud2::ConstPtr& cloud_msg,
        ...
        // ── TF lookup ─────────────────────────────────────────────
        geometry_msgs::TransformStamped tf_cam_vel, tf_map_vel;
        Eigen::Affine3d T_cam_vel = Eigen::Affine3d::Identity();
        try {
            if (img_msg) {
                ros::Time closest_time = img_msg->header.stamp;
                tf_cam_vel = tf_buffer_.lookupTransform(camera_frame_, velodyne_frame_, closest_time);
                T_cam_vel = tf2::transformToEigen(tf_cam_vel);
            }
            tf_map_vel = tf_buffer_.lookupTransform(map_frame_,    velodyne_frame_, lidar_time);
        } catch (tf2::TransformException& ex) {
            ROS_WARN_THROTTLE(1, "TF lookup failed: %s", ex.what());
            return;
        }

        Eigen::Affine3d T_map_vel = tf2::transformToEigen(tf_map_vel);  // map    <- velodyne
        ...
        if (global_map_xyz_->size() >= (size_t)gicp_min_map_size_) {
        ...
        if (img_msg) {
            ...
            // Camera pose in map (GICP-corrected)
            //   T_map_cam_refined = T_map_vel_refined * T_vel_cam
            //                     = T_map_vel_refined * T_cam_vel.inverse()
            // Inverse (camera ← map, refined):
            //   T_cam_map_refined = T_cam_vel * T_map_vel_refined.inverse()
            Eigen::Affine3d T_cam_map_refined = T_cam_vel * T_map_vel_refined.inverse();
            Eigen::Affine3d T_opt_map_refined = T_ros_to_optical * T_cam_map_refined;
            ...
            if (!obj_pts_map.empty()) {
                ...
                for (size_t i = 0; i < img_pts.size(); ++i) {
                    ...
                    const Eigen::Vector3d& p_map = map_pts[i];  // already in refined map frame
                    const auto& src = scan_vel->points[obj_idx[i]];

                    pcl::PointXYZRGB cp;
                    cp.x = p_map.x();  cp.y = p_map.y();  cp.z = p_map.z();
                    cp.r = color[2];   cp.g = color[1];   cp.b = color[0];  // BGR → RGB
