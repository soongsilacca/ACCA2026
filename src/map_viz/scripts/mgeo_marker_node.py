...
class MGeoMarkerNode:
    def __init__(self):
        ...
        package_path = rospack.get_path('map_viz')
        ...
        rospy.loginfo("[MGeo Viz] Publishing map with raw JSON coordinates.")
    ...
    def publish_markers(self):
        ma = MarkerArray()
        # Use Time(0) to bypass TF timestamp buffer issues in RViz
        ...
        # Colors for Nodes
        color_default = make_color(0.2, 0.7, 0.9, 0.5)      # Cyan (Semi-transparent)
        color_stop = make_color(0.95, 0.1, 0.1, 0.9)        # Red (Vivid)
        ...
        # ──────────────────────────────────────────────
        # 2. Visualize Links (using individual LINE_STRIP markers)
        # ──────────────────────────────────────────────
        # Link coloring based on related signal (turn types)
        color_straight = make_color(0.15, 0.55, 0.95, 0.8)    # Electric Blue
        color_left = make_color(0.85, 0.25, 0.9, 0.8)        # Neon Magenta/Purple
        color_right = make_color(0.1, 0.8, 0.4, 0.8)         # Emerald Green
        color_unprotected = make_color(0.95, 0.6, 0.05, 0.8)  # Vivid Amber
        ...
        for link in self.raw_links:
            ...
            # Set non-zero for all scales to prevent rendering issues in some RViz versions
            link_marker.scale.x = 0.4
            link_marker.scale.y = 0.4
            link_marker.scale.z = 0.4
            ...
            if signal == 'straight':
                ...
                c = color_unprotected
            elif signal == 'uturn_normal':
                c = color_left  # group with left
            else:
                c = color_normal
            ...
            # ──────────────────────────────────────────────
            # 3. Add Direction Arrow for each link
            # ──────────────────────────────────────────────
            if self.show_arrows and len(pts) >= 2:
                # Find midpoint segment to place arrow
                mid_idx = len(pts) // 2
                p1 = pts[mid_idx - 1]
                p2 = pts[mid_idx]
                ...
                if dist > 0.1:
                    ...
                    # Scale of the arrow: x=shaft diameter, y=head diameter, z=head length
                    arrow_marker.scale.x = 0.3
                    arrow_marker.scale.y = 0.7
                    arrow_marker.scale.z = 0.8

                    # Semi-transparent white arrow for direction
                    arrow_marker.color = make_color(0.9, 0.9, 0.9, 0.6)

                    # Points defining start and end of the arrow
                    arrow_len = min(1.5, dist)  # keep arrow shorter than segment if segment is tiny
                    nx, ny, nz = dx / dist, dy / dist, dz / dist
                    
                    ap1 = Point(p1[0], p1[1], p1[2])
                    ap2 = Point(p1[0] + nx * arrow_len, p1[1] + ny * arrow_len, p1[2] + nz * arrow_len)
